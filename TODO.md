...
- [ ] Add `aria-label`s to icon-only buttons (add, close, star rating) and `role="alert"`/`aria-live` to notification toasts for screen reader support — icon-button labels done in `fix/accessibility-visual` (`#locate-btn`, `#cancel-btn`, search input, quick-visit note); a persistent `#toast-region[role="status"][aria-live="polite"]` container now exists in `index.html` but `showNotification()` still appends directly to `document.body` instead of into it — wiring that up is the remaining piece (planned for `fix/accessibility-keyboard`, B4)
...
- [x] Clarify or consolidate Decide vs. Places tabs — resolved by removing Places; bottom nav is now Map/Decide/Challenges/You, matching the original design prototypes which never had a Places tab
