...
- [x] Cache gamification `calculateAll()` results instead of recalculating from scratch (incl. O(n²) clustering) on every Challenges/You tab visit — `GamificationUI` now memoizes the result and only invalidates on `MapEngine`'s `created`/`updated`/`deleted` events (`feat/gamification-streaks`)
- [x] Clean up gamification card click listeners on re-render (`gamification-ui.ts`) — replaced per-child listeners with one delegated click/keydown listener per container, bound once in the constructor
...
- [x] Add a short explanation/tooltip on the Challenges tab describing how badges and levels are earned — a native `<details>`/`<summary>` disclosure above the challenge list
...
- [x] Replace the nav drawer's hardcoded placeholder markup (fake progress, two pre-earned badges, avatar "G"/"My Map" in `index.html`, moved here from the old Challenges/You tabs by the navigation redesign) with a real first-render empty state, and render gamification once at boot rather than only on drawer open — done for the challenges/badges/level/streak; a first-place prompt on the empty map itself is still open
